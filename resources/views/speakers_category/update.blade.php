        ...
        <form id="validation-form" action="" method="post">
          ...
          <div class="card-body">
            <div class="row">
              ...
              <div class="col-6">
                <div class="form-group">
                  <label for="ordering">{{ __('admin.ordering') }}</label>
                  <select class="form-control select2bs4 @error('ordering') is-invalid @enderror" name="ordering" style="width: 100%;">
                    <option value="">{{ __('admin.select') }} {{ __('admin.ordering') }}</option>
                    @for($i=1; $i<=30; $i++)
                    <option value="{{$i}}" {{($row->ordering == $i)?'selected':''}}>{{$i}}</option>
                    @endfor
                  </select>
                </div>
              </div>
