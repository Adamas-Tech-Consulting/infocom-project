      ...
      <div class="card">
        ...
        <div class="card-body">
          <table id="list_table" class="table table-bordered table-striped w-100">
            ...
            <tr>
              <td>{{$key+1}}</td>
              <td><span>{{$row->conference_category_name}}</span></td>
              <td>
                {{$row->title}} 
                <div>(<b>{{$row->conference_method_name}} - {{($row->registration_type=='F') ? __('admin.free') : __('admin.paid')}}</b>)<div>
              </td>
              <td>
                <div><b>{{ __('admin.date') }} : </b>{{ date('d M, Y',strtotime($row->conference_start_date))}} - {{ date('d M, Y',strtotime($row->conference_end_date))}}</div>
                @if($row->conference_venue)
                  <div><b>{{ __('admin.venue') }} : </b>{{ $row->conference_venue}}</div>
                @endif
